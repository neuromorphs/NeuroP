...
def get_solution_tsp(cityNames, distanceMatrix, start_city, enforce_start_city):
    ...
    sio.savemat('Q_ISING_TSP_10.mat', {'J': dict_to_mat(J),'h':dict_to_vect(h),'offset':offset})
...
if __name__ == '__main__':
    ...
    sch, distance, sorted_samples_by_energy = get_solution_tsp(cityNames, distanceMatrix, start_city, enforce_start_city)
    ...
    animation = FuncAnimation(fig, partial(draw_solution_tsp, ax=ax, ax2=ax2, cityNames=cityNames, cityCoordinates=cityCoordinates,N=N, sorted_samples_by_energy=sorted_samples_by_energy), frames=100, interval=100, repeat=True)
    animation2 = FuncAnimation(fig2, partial(draw_solution_tsp, ax=ax, ax2=ax2, cityNames=cityNames, cityCoordinates=cityCoordinates,N=N, sorted_samples_by_energy=sorted_samples_by_energy), frames=100, interval=100, repeat=True)
