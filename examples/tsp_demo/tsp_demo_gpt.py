...
class MainWindow(QtWidgets.QMainWindow):
    ...
    #########################################################################
        self.solution_canvas = MplCanvas(self, height=500, width=500, dpi=100)
    ...
    # def connect_coordinates(self,coord1, coord2, map, ax):
    #     x1, y1 = coord1
    #     x2, y2 = coord2
    #     map.drawgreatcircle(x1, y1, x2, y2, linewidth=2, color='b', ax=ax)
    ...
    def plot_solutions(self, sample):
        ...
        # sch_optimal = parse_op_vec_tsp(self.sorted_samples_by_energy[0],N)[1]
        ...
        for i in range(0, len(coordinates_scheduled)):
            ...
            self.connect_coordinates(coord1, coord2, self.map, ax=self.solution_canvas.axes)
        ...
        # pass
    def draw_map(self, lats, longs):
        self.solution_canvas.axes.clear()
        self.map=Basemap(projection='cyl', 
                    llcrnrlon=min(longs)-20, 
                    llcrnrlat=min(lats)-20, 
                    urcrnrlon=max(longs)+20, 
                    urcrnrlat=max(lats)+20)
        self.map.drawcoastlines(color='gray', linewidth=2, ax = self.solution_canvas.axes)
        self.map.fillcontinents(color='lightgreen',lake_color='cornflowerblue', ax = self.solution_canvas.axes)
        self.map.drawcountries(color='gray', linewidth=2, ax = self.solution_canvas.axes)

        for lat, long in zip(lats, longs):
            self.map.scatter(long, lat, marker='o', color='k', ax = self.solution_canvas.axes)
    ...
    def tsp_solver(self,destination_lists, coordinate_lists):
        ...
        distanceMatrix = get_distance_matrix(coordinate_lists)
        # np.save("distanceMatrix_TSP_10.npy", distanceMatrix)

        SOLVE = False
        if not SOLVE:
            sorted_samples_by_energy = np.load("sorted_samples_by_energy.npy")
            # distanceMatrix = np.load("distanceMatrix_TSP_10.npy", allow_pickle=True)

            # for sample in sorted_samples_by_energy:    
            #     m, sch = parse_op_vec_tsp(sample,N)    
            ...
            sch, distance, sorted_samples_by_energy = get_solution_tsp(destination_lists, distanceMatrix, start_city, enforce_start_city)

        self.sorted_samples_by_energy = sorted_samples_by_energy
        self.distanceMatrix = distanceMatrix
        self.N = len(self.distanceMatrix)
        self.samples_buffer = sorted_samples_by_energy.tolist()
        self.data_exists = True
        
         

        # print(json.dumps(problem_info))
        # test=json.dumps(problem_info)
        ...
        # self.update_chat_output(test)
        # dummy response
        # ans = {"path_vector": ["Beijing", "New York", "Los Angeles"]}
    ...
    def update_chat_input(self):
        ...
        messages = [{
                    "role": "system", "content": "You are a helpful assistant that helps to solve combinatorial problems.\
                    ...
                    when the list of destinations are symbolic, such as A, B, C, the list of destinations and list of coordinates are whatever the user has specified.",
        ...
        # Note: the JSON response may not always be valid; be sure to handle errors
            ...
            if function_name == "maxcut_solver":
                function_response = fuction_to_call(
                    adj_matrix=function_args.get("adj_matrix"),
                )
                pass
            elif function_name == "tsp_solver":
        ...
        update_info="-----FLAG: INVOKED SOLVERS-----"
        # self.update_chat_output(update_info)
    ...
    def update_chat_output(self,second_response):
        ...
        self.chat_output.appendPlainText(second_response)
            # Find the last calculation result in the reply
    #     calculation_result = ""
    #     lines = reply.split("\n")
    #     for line in reversed(lines):
    #         if "=" in line:
    #             calculation_result = line.strip()
    #             break

    # # Append the calculation result to the chat output if it exists
    #     if calculation_result:
            
            # self.chat_output.appendPlainText(calculation_result)
    ...
    def update_map(self):
        ...
        if not self.samples_buffer:
            ...
            if self.data_exists:
                self.draw_map(self.lats, self.longs)
